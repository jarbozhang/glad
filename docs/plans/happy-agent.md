...
- **Server API**: REST endpoints at `https://happy.bfelab.com` + Socket.IO at `/v1/updates`
...
- [x] Create `src/config.ts` — reads `HAPPY_SERVER_URL` (default: `https://happy.bfelab.com`), `HAPPY_HOME_DIR` (default: `~/.happy`), derives credential file path as `${happyHomeDir}/agent.key`
